...
const registerSchema = z
  .object({
    ...
    // Admin — required when role === "admin", validated by the refine below
  ...
  // Admin: invite code required and must match expected format
  .refine(
    (d) =>
      d.role !== "admin" ||
      (typeof d.adminInviteCode === "string" &&
        d.adminInviteCode.trim().length > 0),
    {
...
export function Register() {
  ...
  const onSubmit = (data: RegisterFormValues) => {
    ...
    registerMutate(payload, {
      onError: (error) => {
        // Surface backend invitation errors onto the form field
        const msg = (error as Error).message ?? "";
        if (
          msg.includes("invitation code") ||
          msg.includes("expired") ||
          msg.includes("already been used")
        ) {
          setError("adminInviteCode", { message: msg });
          // Go back to step 2 so the user can see the error
          setStep(2);
        }
      },
    });
        ...
        <Card className="shadow-lg">
          ...
          <CardContent>
            <form onSubmit={handleSubmit(onSubmit)} noValidate>
              ...
              {step === 0 && (
                <div className="space-y-4">
                  <div className="space-y-1.5">
                    ...
                    <Input
                      id="name" placeholder="Jane Doe" autoComplete="name"
                      {...register("name")}
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      id="email" type="email" placeholder="jane@example.com" autoComplete="email"
                      {...register("email")}
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      ...
                      <button type="button" onClick={() => setShowCpw(v => !v)}
                        className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground"
                        tabIndex={-1} aria-label={showCpw ? "Hide" : "Show"}>
              ...
              {/* Step 1 */}
              {step === 1 && (
                <RoleSelector value={selectedRole} onChange={(r) => setValue("role", r)} />
              )}
              ...
              {step === 2 && (
                <div className="space-y-6">
                  ...
                  <div className="border-t pt-4">
                    {selectedRole === "student" && <StudentFields register={register} errors={errors} disabled={isPending} />}
                    {selectedRole === "teacher" && <TeacherFields register={register} errors={errors} disabled={isPending} />}
                    {selectedRole === "admin"   && <AdminFields   register={register} errors={errors} disabled={isPending} />}
                  </div>
                </div>
              )}
