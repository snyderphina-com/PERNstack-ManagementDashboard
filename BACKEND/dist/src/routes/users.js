...
router.get("/:id/departments", async (req, res) => {
    try {
        ...
        const countResult = userRecord.role === "teacher"
            ? await db
                .select({ count: sql `count(distinct ${departments.id})` })
                .from(departments)
...
router.get("/:id/subjects", async (req, res) => {
    try {
        ...
        const countResult = userRecord.role === "teacher"
            ? await db
                .select({ count: sql `count(distinct ${subjects.id})` })
                .from(subjects)
                .leftJoin(classes, eq(classes.subjectId, subjects.id))
                .where(eq(classes.teacherId, userId))
            : await db
        ...
        const subjectsList = userRecord.role === "teacher"
            ? await db
                ...
                .leftJoin(classes, eq(classes.subjectId, subjects.id))
                .where(eq(classes.teacherId, userId))
                .groupBy(subjects.id, subjects.departmentId, subjects.name, subjects.code, subjects.description, subjects.createdAt, subjects.updatedAt, departments.id, departments.code, departments.name, departments.description, departments.createdAt, departments.updatedAt)
